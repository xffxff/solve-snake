...
class PPONet(object):
    ...
    def _cnn(self, x, out_dim):
        x = layers.conv2d(x, filters=8, kernel_size=3, strides=(2, 2), activation=tf.nn.tanh)
        x = layers.conv2d(x, filters=16, kernel_size=2, strides=(1, 1), activation=tf.nn.tanh)
        x = layers.flatten(x)
