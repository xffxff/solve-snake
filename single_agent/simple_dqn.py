...
from utils.wrappers import FrameStack
...
class DQNNet(object):
    ...
    def __init__(self, obs, act_n):
        out = layers.conv2d(obs, filters=8, kernel_size=3, strides=(2, 2), activation=tf.nn.relu)
        out = layers.conv2d(out, filters=16, kernel_size=2, strides=(1, 1), activation=tf.nn.relu)
...
class DQNAgent(object):
    ...
    def _create_network(self):
        with tf.variable_scope('main'):
            obs_float = tf.cast(self.obs_ph, tf.float32)
            net = DQNNet(obs_float, self.act_n)
            self.q_acts = net.network_output()
        with tf.variable_scope('target'):
            next_obs_float = tf.cast(self.next_obs_ph, tf.float32)
    ...
    def load_model(self, checkpoints_dir, index=None):
        if index is None:
            index = get_latest_check_num(checkpoints_dir)
        self.saver.restore(self.sess, osp.join(checkpoints_dir, 'tf_ckpt-{}'.format(index)))
...
class DQNRunner(object):
    ...
    def run_test_and_render(self, model):
        ...
        logger = EpochLogger()
        self.agent.load_model(self.checkpoints_dir, model)
